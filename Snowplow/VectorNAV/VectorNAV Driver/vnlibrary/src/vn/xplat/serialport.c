...
VnError VnSerialPort_open_internal(VnSerialPort *serialport, char const *portName, uint32_t baudrate, bool checkAndToggleIsOpenFlag)
{
	...
	if (checkAndToggleIsOpenFlag && VnSerialPort_isOpen(serialport))    //--> If already open
	...
	portFd = open(
		...
		O_RDWR | O_NOCTTY);                             //--> What about O_NDELAY?
		#elif __APPLE__
	...
	memset(
		&portSettings,
		0,
		sizeof(portSettings));  //Initiate new termios struct to 0
	...
	portSettings.c_cflag = baudrateFlag;                                        //--> That's how you can set baudrate? --> Well, you can start off with that, yeah. Makes sense. You can start with any.
...
VnError VnSerialPort_close_internal(VnSerialPort *serialport, bool checkAndToggleIsOpenFlag)
{
	if (checkAndToggleIsOpenFlag && !VnSerialPort_isOpen(serialport))   //--> If already closed.
		return E_INVALID_OPERATION;
