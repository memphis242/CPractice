...
#define NUM_OF_PORTS  38    //"/dev/cuau1"


int Cport[NUM_OF_PORTS], error;
struct termios currentconfig, old_port_settings[NUM_OF_PORTS];
char *comports[NUM_OF_PORTS] = {"/dev/ttyS0","/dev/ttyS1","/dev/ttyS2","/dev/ttyS3","/dev/ttyS4","/dev/ttyS5",
...
typedef enum { ttyS0, ttyS1, ttyS2, ttyS3, ttyS4, ttyS5, ttyS6, ttyS7, ttyS8, ttyS9, ttyS10, ttyS11, ttyS12, ttyS13, ttyS14, ttyS15,
                ...
                cuaU0, cuaU1, cuaU2, cuaU3} PORTTYPE;



//The mode char array contains the configuration settings for the termios structure, like number of data bits, parity, and so on.
int RS232_OpenComport(int comport_number, int baudrate, const char *mode) {
    
    //Check comport_number validity
    if( (comport_number >= NUM_OF_PORTS) || (comport_number < 0) ) {
        puts("Illegal comport number");
        return(-1);
    }
    ...
    switch(baudrate) {
        ...
        case 4000000 : baudr = B4000000;
                       break;
        default      : puts("Invalid baudrate");
                       return(-1);
                       break;
    }

    int num_databits = CS8, cpar = 0, ipar = IGNPAR, bstop = 0;    //Sets number of data bits to 8, no parity, 
    ...
    switch(mode[0]) {
        case '8': num_databits = CS8;
                  break;
        case '7': num_databits = CS7;
                  break;
        case '6': num_databits = CS6;
                  break;
        case '5': num_databits = CS5;
                  break;
    ...
    */
    ...
    //Open in Read/Write mode, without being controlling terminal, and regardless of DCD signal --> Most common way to open a file.
    ...
    //Initialize new termios structure to hold new settings
    memset(&currentconfig, 0, sizeof(currentconfig));  /* clear the new struct */

    //Set up values in new structure
    currentconfig.c_cflag = num_databits | cpar | bstop | CLOCAL | CREAD;
    currentconfig.c_iflag = ipar;
    currentconfig.c_oflag = 0;  //Raw output
    currentconfig.c_lflag = 0;  //Raw input
    currentconfig.c_cc[VMIN] = 0;      /* block untill n bytes are received */
    currentconfig.c_cc[VTIME] = 0;     /* block untill a timer expires (n * 100 mSec.) */

    //Set baud rate
    cfsetispeed(&currentconfig, baudr);
    cfsetospeed(&currentconfig, baudr);

    //Initialize the file with the new termios structure
    error = tcsetattr(Cport[comport_number], TCSANOW, &currentconfig);
    if(error == -1) {
...
HANDLE Cport[NUM_OF_PORTS];


char *comports[NUM_OF_PORTS]={"\\\\.\\COM1",  "\\\\.\\COM2",  "\\\\.\\COM3",  "\\\\.\\COM4",
                              "\\\\.\\COM5",  "\\\\.\\COM6",  "\\\\.\\COM7",  "\\\\.\\COM8",
